...
- Measure accuracy: offline event-level eval harness plus live human-review precision and recall metrics.
...
| `/jobs/{jobId}` | `ReplayClient.tsx` | Completed-job replay with synchronized overlays, jump-to-violation, and Report Miss controls |
...
| POST | `/jobs/{job_id}/violations/manual` | File a reviewer-observed missed violation |
| GET | `/metrics/review` | Human-review precision and recall metrics |
...
`GET /api/metrics/review` aggregates human review decisions into precision and recall metrics — `overall`, per `jobs`, and per `confidence_bands` (helmet confidence buckets `under_50`, `50_to_65`, `65_to_80`, `80_plus`). Precision is `confirmed / (confirmed + false_positive)` over pipeline-detected records. Recall is `confirmed / (confirmed + manual)`, where `manual` counts reviewer-reported misses; because reviewers only report misses they notice, this is an upper bound on true recall.

### Manual violation reports

`POST /api/jobs/{job_id}/violations/manual` accepts a video timestamp and/or frame number plus optional reviewer notes and plate text. It seeks the stored video to that moment, saves an annotated evidence frame, and creates a confirmed violation with `source = "manual"`.

The backend diagnoses a `miss_reason` from nearby sampled detection metadata:

- `not_sampled`
- `motorcycle_not_detected`
- `helmet_model_miss`
- `gated_or_undervoted`
- `no_metadata`

This separates sampling misses, detector misses, and pipeline-gating misses so each can feed the appropriate tuning or fine-tuning work.
...
erDiagram
    ...
    VIOLATIONS {
        ...
        TEXT source "detected|manual"
        TEXT note "nullable reviewer note"
        TEXT miss_reason "nullable diagnosis"
    }
```
...
Saved violations are numbered per job in frame order. Each evidence image highlights its specific rider with a magenta outline and track label, and jumping to a violation highlights the same track on the replay overlay. The replay page also lets a reviewer report a violation the pipeline missed.
...
Turns the human decisions collected on the Violations page into live precision and recall metrics — overall, per job, and per helmet-confidence band. Confirmed/false-positive decisions measure saved-record precision; reviewer-reported misses measure recall and identify valuable fine-tuning examples that the current model failed to detect.
...
### 2026-07-07 — Review loop and per-record evidence

1. Evidence screenshots and replay overlays highlight the specific rider for each numbered violation record.
2. Reviewers can report missed violations from replay; the backend captures evidence and diagnoses the miss from sampled metadata.
3. Review metrics now report recall from manual misses alongside precision from confirmed/false-positive decisions.
4. The review queue and CSV export expose record source, miss diagnosis, and reviewer notes.
